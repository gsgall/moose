...
ADConductiveCurrent::validParams()
{
  ...
  params.addParam<MaterialPropertyName>(
      "ang_freq", "ang_freq", "The drive frequency (only real component for now).");
  params.addParam<MaterialPropertyName>(
      "permeability", "mu_vacuum", "The permeability (only real component for now).");
...
ADConductiveCurrent::ADConductiveCurrent(const InputParameters & parameters)
  : ADVectorKernel(parameters),
    ...
    _omega(getADMaterialProperty<Real>("ang_freq")),
    _mu(getADMaterialProperty<Real>("permeability")),
...
ADConductiveCurrent::computeQpResidual()
{
  ...
  if (_component == EM::REAL)
  {
    return _test[_i][_qp] * _mu[_qp] * _omega[_qp] * -1.0 *
           (_cond_imag[_qp] * _E_real[_qp] + _cond_real[_qp] * _E_imag[_qp]);
  }
  else
  {
    return _test[_i][_qp] * _mu[_qp] * _omega[_qp] *
           (_cond_real[_qp] * _E_real[_qp] - _cond_imag[_qp] * _E_imag[_qp]);
  }
}

// Real
// ADConductiveCurrent::computeQpJacobian()
//{
//   return 0.0;
// }
